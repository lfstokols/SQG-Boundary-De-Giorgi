...
Since each $\theta_n$ and $\uhigh^n + \ulow^n$ is a suitable pair on $[-5,0]\times\Omega_n$ by assumption, we can apply the inequality \eqref{local condition} to find that, for some constant $C$ independent of $n$ and of $\varphi$, on $[-4,0]\times \Omega_n$
...
\\ & + C \norm{\varphi}_{C^0(C^1)} \paren{ 1 + \int_{-5}^0 \int \theta_+ \paren{\ulow^n - \dot{\Gamma}_n + \uhigh^n} \cdot \grad\psi }.
\end{aligned}\end{equation}
...
\int_{-5}^0\!\! \int \uhigh^n \cdot (\theta_+ \grad\psi) &\leq C.
\end{aligned} \end{equation}
...
Before proving Theorem~\ref{thm:main continuity}, we will show an essentially equivalent lemma.  

\begin{lemma}[Continuity of suitable solutions] \label{thm:continuity for suitable}
Let $\Omega \subseteq \R^2$ be an open, bounded domain with $C^{2,\beta}$ boundary, $\beta \in (0,1)$.  Let $k > 0$ be a constant.  Then there exist constants $\alpha \in (0,1)$ and $C > 0$ such that the following holds:

Let $\theta:[-5,0] \times \Omega \to \R$ and $u:[-5,0]\times\Omega \to \R^2$ be a suitable solution to \eqref{eq:main linear}.  Assume that $\norm{\theta}_{L^\infty([-5,0]\times\Omega)} \leq 2$ and that $u$ is the Riesz transform of an $L^\infty$ function, i.e. $\norm{\Lambda^{-1} \grad^\perp \cdot u}_{L^\infty([-5,0] \times \Omega)} \leq k$.  

Then for any point $P \in \bar{\Omega}$, $\theta$ is H\"{o}lder continuous at $(0,P)$ and
\[ \sup_{(t,x) \in [-5,0]\times\Omega} \frac{|\theta(t,x) - \theta(0,P)|}{(|t|^2 + |x|^2)^{\alpha/2}} \leq C. \]
...
\[ u = \grad^\perp \Lambda^{-1} \bracket{ \Lambda^{-1} \grad^\perp \cdot u} = \sum_{j=j_0}^\infty u_j \]
for a sequence $(u_j)_{j\geq j_0}$ of divergence-free functions calibrated with some constant $\Ccalib = \Ccalib(\Omega, k)$ and center 0.  Assume without loss of generality that $j_0 < 0$.
...
In Proposition~\ref{thm:existence} we construct global-in-time solutions to \eqref{eq:main nonlinear}.  If $t > 5$, then observe that the property of being a suitable solution to \eqref{eq:main linear} is linear in $\theta$ and translation-invariant, so 
