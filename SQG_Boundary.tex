...
with the sum converging in the $L^2$ sense.  Assume that $(u_j)_{j \in \Z}$ is a calibrated sequence with constant $\Ccalib$ and some center, and that $\div(u_j)=0$.  
...
To that end, we shall consider a family of functions $\theta:[-T,0]\times\Omega \to \R$, $\ulow$ and $\uhigh:[-T,0]\times \Omega \to \R^2$, and paths $\Gamma$ and $\gamma:[-5,0] \to \R^2$ which satisfy
...
Let $\Ccalib$, $\Comega$, $\Cgamma$, $T$, and $R$ be positive constants, and let $\psi:\R^2 \to \R$ be a function with bounded first and second derivatives. Then there exists a constant $C>0$ such that the following holds:
...
\[ \int \uhigh \theta_+ \grad \psi(x - \gamma(t)) \,dx \leq C(\psi,\eps) \Ccalib \paren{\int \indic{\theta_+>0} + \int \theta_+^2} + \eps \int \abs{\Lambda^{1/2} \theta_+}^2. \]
