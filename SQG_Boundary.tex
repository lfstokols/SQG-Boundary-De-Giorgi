...
In Section \ref{sec:littlewood paley} we will show that $u$ is calibrated and in Section \ref{sec:holder} we will show that it remains calibrated at all scales (specifically, with fixed constant $\Ccalib$ but not always with the same center $N$).  Therefore we will consider the linear equation
...
For any vector $v = (v_1,v_2)$, by $v^\perp$ we mean $(-v_2,v_1)$, and by $\grad^\perp$ we mean $(-\del_y, \del_x)$.  The differential operators $\grad$ and $D^2$ do not include time-derivatives.  
...
Let $\Ccalib$, $\Comega$, and $\Cgamma$, be positive constants. Then there exists a constant $\mu>0$ such that the following holds:
...
\begin{itemize}
\item Consider the low-pass term.  As in the proof of Lemma~\ref{thm:energy inequality}, we have $|\ulow^n(t,x) - \dot{\Gamma}_n(t)| \leq (1+8\Ccalib)\Cgamma + 6 \Ccalib$ for $t \in [-4,0]$ and $x \in \supp(\theta_+) \subseteq \Omega_n \cap B_3(\Gamma_n(t))$.  Thus for $t \in [-4,0]$ we have for $C$ independent of $n$ and of $\varphi$
...
Then for any $\eps \in (0,1/3)$ such that
...
%In Section~\ref{sec:holder}, when we apply this lemma to the proof of Theorem~\ref{thm:main continuity}, the parameter $\Cgamma$ will be defined in terms of the scaling parameter $\eps$.  However, in the present lemma, the set of acceptable scaling parameters depends on the value of $\Cgamma$.  To avoid circularity, this dependence must be made explicit in the assumption \eqref{Cgamma and eps for harnack}.  It is for this reason that a shorter, non-constructive proof of the existence of an acceptable parameter $\eps$ would be insufficient.  

%The idea of the proof is to consider a small enough time interval that $\Gamma(t) + \gamma(t)$ is very close to $\Gamma(t)$.  This is possible because $\gamma$ is Lipschitz by assumption.  However, special attention must be paid to the dependence of the parameters $\eps$, $\Cgamma$, and $k_0$ to avoid a circular definition.  The quantity $k_0$ depends on $\Cgamma$ through the non-constructive proof in Lemma~\ref{thm:oscillation general}, and the quantity $\Cgamma$ will be defined constructively in Section~\ref{sec:holder} terms of the scaling parameter $\eps$.  

%Note, therefore, that in this lemma the scaling parameter $\eps$ does not depend on $k_0$, and depends on $\Cgamma$ only through the constructive relation \eqref{Cgamma and eps for harnack}.  
%To avoid circularity, 

%In the following proof, the scaling parameter $\eps$ depends on $\Cgamma$, which might lead to circularity.  That dependence, expressed in \eqref{Cgamma and eps for harnack}, is constructive

%Ostensibly, $k_0$ will depend on the constant $\Cgamma$ (since the value $\Cgamma$ is important in the proof of Lemma~\ref{thm:oscillation general} where $k_0$ is calculated) but in Section~\ref{sec:holder} we will actually define $\Cgamma$ in terms of the scaling parameter $\eps$, which means we cannot require $\eps$ to depend on $k_0$.  

%The idea of the proof is to consider a small enough time interval that $\Gamma(t) + \gamma(t)$ is very close to $\Gamma(t)$.  This is possible because $\gamma$ is Lipschitz by assumption.  However, considerable attention must be paid to the dependence of the parameters $\eps$, $\Cgamma$, and $k_0$.  Ostensibly, $k_0$ will depend on the constant $\Cgamma$ (since the value $\Cgamma$ is important in the proof of Lemma~\ref{thm:oscillation general} where $k_0$ is calculated) but in Section~\ref{sec:holder} we will actually define $\Cgamma$ in terms of the scaling parameter $\eps$, which means we cannot require $\eps$ to depend on $k_0$.  
...
with that sum converging in $L^2(\Omega)$ and $(u_j)_j$ calibrated with constant $\Ccalib$ and center $N$.  Suppose that on $\Omega$ the functions $K_{1/4}$ and $K_1$ (defined in Proposition~\ref{thm:Caff Stinga representation}) satisfy the relation
...
Moreover, $(u_j)_j$ is calibrated with the same constant $\Ccalib$ but with center $N - \log_2(\eps)$, and the estimate
...
for a sequence $(u_j)_{j\geq j_0}$ of divergence-free functions calibrated with some constant $\Ccalib = \Ccalib(\Omega)$ and center 0.  
...
%\[ 5 \max\paren{ - \Ccalib \log_2(\eps) e^{10\eps\Ccalib}, (1-j_0) \Ccalib} \leq \eps^{-a k}/k \qquad \forall k\geq 0. \]
...
\end{align*}
The function $x \mapsto \ulowth{k-1}(\eps t, \eps x)$ is Lipschitz, with Lipschitz constant less than $2 \eps \Ccalib$.  Moreover, each $u_j$ has $\norm{u_j}_\infty \leq \Ccalib$.  Thus from the above calculation we can bound
\begin{equation} \label{ODE for dot gamma} |\dot{\gamma}_k(t)| \leq 2 \eps \Ccalib |\gamma_k(t)| - \Ccalib \log_2(\eps). \end{equation}
...
Trivially $|\dot{\gamma}_0| \leq (j_0-1) \Ccalib$, so if we define
\[ \Cgamma = \max\paren{ - \Ccalib \log_2(\eps) e^{10\eps\Ccalib}, (j_0-1) \Ccalib} \]
