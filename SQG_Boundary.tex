...
\begin{definition}
A pair $\theta, u \in L^\infty(0,T;L^2(\Omega))$ is called suitable if, for any smooth non-negative functions $\Psi \in C^\infty([0,T]\times \Omega)$ and $\varphi \in C^\infty_0(0,T;C^\infty(\Omega))$, there exists a constant $C$ depending on $\norm{\grad\Psi}_{L^\infty([0,T]\times\Omega)}$ and $\sup_{[0,T]} \bracket{\Psi(t,\cdot)}_{1/4}$ such that the function $\theta_+ := (\theta-\Psi)_+$ satisfies the inequalities
\begin{equation}\label{leray-hopf condition}
\ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \theta_- \leq C \paren{ \int \indic{\theta_+ \geq 0} + \abs{\int \theta_+ (\del_t \Psi + u\cdot\grad\Psi)} }
\end{equation}
and
...
+ \sup_{[0,T]} \int \theta_+^2 + \int_0^T |\{\theta \geq 0\}|^2 + 1} 
...
Moreover, for any non-negative function $\varphi \in C^0(0,T;C^1(\Omega))$,
...
In particular, $\theta$ and $u$ are a suitable pair.
...
\begin{equation}\label{first energy inequality} \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \theta_- \leq C \paren{ \abs{\int \theta_+ (\del_t\Psi+u\cdot\grad\Psi)} + \int \indic{\theta_+>0} }.\end{equation}
...
\[ \norm{\theta_+}_{L^2(0,T;\HD^{1/2})}^2 \leq C \paren{\norm{\theta_+}_{L^\infty(0,T;L^2(\Omega))}^2 + \iint \indic{\theta_+ \geq 0} - \iint \theta_+ \abs{\del_t\Psi + u\cdot\grad\Psi} }. \]
...
+ \sup_{[0,T]} \int\! \theta_+^2 + \int_0^T\! |\{\theta_+\geq 0\}| + \int_0^T\! |\{\theta \geq 0\}|^2}. \end{aligned}\]
