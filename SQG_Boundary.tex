...
In order to apply De Giorgi's method to weak solutions of \eqref{eq:main linear}, we will need to assume a certain a priori estimate which holds, in particular, for $L^2(H_0^1)$ weak solutions.  However, such solutions are only known to exist for short time and for $H^2$ initial data, as shown by Constantin and Nguyen in \cite{CoNg.strong}.  We call weak solutions in $L^2(\HD^{1/2})$ which happen to verify this a priori estimate ``suitable solutions,'' by analogy to suitable solutions to Navier-Stokes as in [cite].  We give the formal definition of suitable solutions in Section~\ref{sec:suitable}, where we also construct global-in-time suitable solutions using the vanishing viscosity method.  These solutions are not a priori more regular than those constructed in \cite{CoIg.fraclap}, but because they verify an energy inequality we are able to apply the De Giorgi method.  
\vskip.3cm

The Paper is organized as follows. Section \ref{sec:lambda} is dedicated to basic properties of the operator $\Lambda$ and the corresponding Sobolev spaces $\HD^s$.  In Section~\ref{sec:suitable} we construct weak solutions which verify the suitability condition.  %In Section~\ref{sec:Linfty} we prove $L^2$ to $L^\infty$ regularization.  
...
In this section, we will define and then construct suitable solutions to \eqref{eq:main nonlinear}.  
...
Let $\Omega \subseteq \R^2$ be bounded and open with $C^{2,\beta}$ boundary, $\beta \in (0,1)$, and let $0 < S < T$ be times.  Let $\theta, u$ be a solution to \eqref{eq:main linear} on $\Omega \times [0,T]$, with $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; H_0^1(\Omega))$ and $u \in L^\infty(0,T; L^2(\Omega)) \cap L^4(0,T; L^4(\Omega))$.  

Then for any smooth non-negative function $\Psi \in C^\infty([0,\infty)\times \R^2)$ satisfying $\norm{\grad\Psi}_{L^\infty([0,\infty)\times\R^2)} \leq k$ and the H\"{o}lder seminorm $\sup_{[0,\infty)} \bracket{\Psi(t,\cdot)}_{1/4; \R^2} \leq k$ for some constant $k$ and any smooth non-negative $\varphi \in \Ctest(S,T;C^\infty(\Omega))$, the function $\theta_+ := \paren{\theta - \Psi}_+$ satisfies
...
\frac{-1}{2} \int_S^T\!\!\!\!\int\! \theta_+^2 \del_t \varphi &\leq -\int_S^T\!\!\!\!\int\! \varphi \theta_+ \paren{\del_t \Psi + u\cdot\grad\Psi} + \frac{1}{2} \int_S^T\!\!\!\!\int\! \theta_+^2 u\cdot\grad\varphi
\\ & \hskip2cm - \int_S^T\!\!\!\!\iint_{\Omega \times \Omega} [\varphi(x) \theta_+(x) - \varphi(y) \theta_+(y)][\Psi(x) - \Psi(y)] K_1(x,y) \,dxdy
\\ & \hskip2cm - \frac{1}{2} \int_S^T\!\!\!\!\iint_{\Omega \times \Omega} [\theta_+(x)^2 - \theta_+(y)^2][\varphi(x) - \varphi(y)] K_1(x,y) \,dxdy.
\end{aligned}
%\begin{aligned}
...
Combining this with \eqref{first form of cacciopolli} and \eqref{good lower bound on psi term cacciopoli} we obtain \eqref{leray-hopf condition}.  
...
\\ &\leq \frac{-1}{2} \iint_{\Omega \times \Omega} [\theta_+(x)^2 - \theta_+(y)^2][\varphi(x)-\varphi(y)] K_1(x,y) \,dxdy.
\end{aligned} \end{equation*}

From the Caffarelli-Stinga Representation of Proposition~\ref{thm:Caff Stinga representation},
\[ - \int \varphi \theta_+ \Lambda \Psi \leq - \iint_{\Omega \times \Omega} [\varphi(x) \theta_+(x) - \varphi(y) \theta_+(y)][\Psi(x) - \Psi(y)] K_1(x,y) \,dxdy.  \]

With these two bounds, the stated result follows from \eqref{equality with varphi}.  
%For the $\Lambda \Psi$ term, we can ignore the boundary term of the Caffarelli-Stinga Representation from Proposition~\ref{thm:Caff Stinga representation} because it is non-negative so
%\[ \int \varphi \theta_+ \Lambda \Psi \geq \iint [\varphi(x) \theta_+(x) - \varphi(y) \theta_+(y)] [\Psi(x) - \Psi(y)] K(x,y). \]
...
%
%From the bound \eqref{first energy inequality}, we know that 
%\[ \norm{\theta_+}_{L^2(t,T;\HD^{1/2})}^2 \leq C \paren{\int_t^T\norm{\theta_+}_{L^2}^2 + k^2 \iint \indic{\theta_+ \geq 0} + \abs{\iint \theta_+ \paren{\del_t\Psi + u\cdot\grad\Psi} }}. \]
%\[ \norm{\theta_+}_{L^2(0,T;\HD^{1/2})}^2 \leq C \paren{\norm{\theta_+}_{L^\infty(0,T;L^2(\Omega))}^2 + k^2 \iint \indic{\theta_+ \geq 0} + \abs{\iint \theta_+ \paren{\del_t\Psi + u\cdot\grad\Psi} }}. \]
%
%We conclude that
%\[\begin{aligned} \frac{1}{2} \iint\!\! \varphi \del_t \theta_+^2 &\leq \frac{1}{2} \iint\!\! \theta_+^2 u\cdot\grad\varphi + \abs{\iint\!\! \varphi \theta_+ \paren{\del_t\Psi+u\cdot\grad\Psi}} 
%+ C\norm{\varphi}_{C^0(0,T;C^{0,1})} \Bigg(\iint\!\! \theta_+^2 + \sup_{[0,T]} \int\!\! \theta_+^2 
%\\ & + k^2 \int_0^T\!\! |\{\theta_+\geq 0\}| + \abs{\iint\!\! \theta_+ \paren{\del_t\Psi + u\cdot\grad\Psi}} + k^4 \int_0^T\!\! |\{\theta_+ \geq 0\}|^2 \Bigg). 
%\end{aligned}\]
\end{proof}
\vskip.3cm
...
\begin{definition}
A pair $\theta, u$ is called a \textbf{suitable solution} on a space time domain $[S,T]\times \Omega$ if $S < T$ are finite, $\Omega \subseteq\R^2$ open and bounded,  $\theta,u \in L^\infty(0,T;L^2(\Omega)$, $\theta \in L^2(0,T;\HD^{1/2}(\Omega))$, $u \in L^4(0,T; L^4(\Omega))$ and
\begin{enumerate}
...
%it satisfies \eqref{leray-hopf condition} and \eqref{local condition} at all scales with the same universal constant $\Csuit$ defined in Proposition~\ref{thm:suitability}.  Specifically, if the following holds:
...
Define $\tilde{\Omega} := \{x \in \R^2: \mu x \in \Omega\}$, $\tilde{\theta}_+(t,x) := (\lambda \theta(\mu t, \mu x)-\Psi(t, x))_+$, and $\tilde{u}(t,x):= u(\mu t, \mu x)$.  Let $\varphi \in \Ctest(S,T;C^\infty(\tilde{\Omega}))$ be non-negative.  

Then $\tilde{\theta}_+$ and $\tilde{u}$ and $\varphi$ and $\Psi$ satisfy \eqref{leray-hopf condition} and \eqref{local condition} on $[\mu\n S,\mu\n T] \times \tilde{\Omega}$ with the universal constant $\Csuit$.  
\end{enumerate}
\end{definition}
...
By Peano's existence theorem for ODEs, solutions to this system exist on some interval $[0,T]$ where $T$ depends on $\Omega$ and $N$ and the $L^2$ norm of the initial data.  
...
\[ \ddt \int \frac{1}{2} (\theta - a)_+ + \int \abs{\Lambda^{1/2} (\theta - a)_+}^2 = \frac{-1}{2}\int u \cdot \grad (\theta - a)_+^2 - \int a \Lambda (\theta - a)_+ + \eps \int \abs{\grad (\theta - a)_+}^2. \]
\[ = - \int a (\theta - a)_+ B_1 \leq 0. \]
This inequality is scaling-invariant, so the same holds for $\lambda \theta(\mu t, \mu(\eps)$ for any $\lambda, \mu > 0$.  

By the standard De Giorgi argument (see Lemma~\ref{thm:DG1 skeleton} in the Appendix for details), there exists a universal constant $\delta$ such that $\int_0^2 \int (\lambda \theta(\mu t, \mu x)_+^2 \,dxdt \leq \delta$ implies $\theta \leq \lambda\n$ on $[\mu 1,\mu 2]$.  In fact, by comparison with a constant solution, $\theta \leq \lambda\n$ on $[\mu, \infty)$.  Taking $\lambda = \sqrt{\frac{\delta}{2 \mu^{-2} \norm{\theta_0}_{L^2(\Omega)}^2}}$, we find $\theta(t,\cdot) \leq C t^{-1} \norm{\theta_0}_{L^2(\Omega)}$ for a universal constant $C$.  

guarantees that $\theta \in L^\infty([t,\infty]\times\Omega)$ for all $t > 0$, with norm depending on $t$ and $\norm{\theta_0}_{L^2(\Omega)}$ and $\Omega$ but independent of $\eps$.  

In the case $S \geq 2$, we can apply \eqref{leray-hopf condition} to establish the family of energy inequalities assumed in Lemma~\ref{thm:DG1 skeleton} and then apply Lemma~\ref{thm:DG1 skeleton} to the function $\frac{\sqrt{\delta}}{\sqrt{2} \norm{\theta_0}_2} \theta(t+2,x)$ to show that 
\[ \norm{\theta}_{L^\infty([2,T]\times\Omega)} \leq \paren{\frac{2}{\delta}}^{1/2} \norm{\theta_0}_2. \]

For small $S$, we can apply the above argument to the function $\theta\paren{(S/2)t, (S/2)x}$.  
\end{proof}
...
Given an open, bounded domain $\Omega \subseteq \R^2$ with $C^{2,\beta}$ boundary, $\beta \in (0,1)$, and initial data $\theta_0 \in L^2(\Omega)$, there exists a global-in-time weak solution $\theta$ to \eqref{eq:main nonlinear} such that, for any $0 < S < T < \infty$, $\theta$ and $u := \grad^\perp \Lambda^{-1} \theta$ are a suitable pair on $[S,T] \times \Omega$.  
...
Therefore $\del_t \theta_\eps$ is uniformly bounded in $L^2(\HD^{-3/2})$.  By Aubin-Lions, the sequence $\theta_\eps$ has a strong limit in $L^2(L^2)$.  Call this limit $\theta$.  
...
It remains only to prove that $\theta$ and $u$ are a suitable pair.  This will be true because, for $\theta_\eps$ and $u_\eps$, we can make the same calculation as in the proof of Proposition~\ref{thm:suitability}.  The added viscosity term produces error terms which vanish as $\eps \to 0$, and since the suitability conditions \eqref{leray-hopf condition} and \eqref{local condition} are maintained by $L^2$ limits, the limiting functions $\theta$ and $u$ will satisfy the suitability conditions with no error terms.  The details of this calculation are given below.  

Let $0 < S < T < \infty$ be constant times, and let $\Psi$, $\varphi$, $\lambda$, and $\mu$ be as in the definition of suitable solutions.  Define 
\newcommand{\thetae}{\tilde{\theta}_\eps}
\newcommand{\ue}{\tilde{u}_\eps}
\newcommand{\thetaep}{\tilde{\theta}_{\eps,+}}
\begin{align*}
\tilde{\Omega} &:= \{x \in \R^2: \mu x \in \Omega\}, \\
\tilde{T} &:= \mu\n T, \\
\tilde{S} &:= \mu\n S, \\
\thetae(t,x) &:= \lambda\theta_\eps(\mu t, \mu x), \\
\ue(t,x) &:= u_\eps(\mu t, \mu x), \\
\thetaep(t,x) &:= \paren{\thetae(t, x) - \Psi(t,x)}_+.
\end{align*}
...
Similarly for the second suitability condition, we have as in the proof of Proposition~\ref{thm:suitability} (all $\iint$ are $\int_{\tilde{S}}^{\tilde{T}} \int_{\tilde{\Omega}}$)
\begin{equation} \label{local for eps positive} \begin{aligned}
\frac{-1}{2} \iint \thetaep^2 \del_t \varphi &= \frac{1}{2} \iint \thetaep^2 \ue \cdot \grad\varphi - \iint \varphi \thetaep \paren{\del_t \Psi + \ue \cdot\grad\Psi}  
\\ & \hskip2cm - \int_{\tilde{S}}^{\tilde{T}}\!\!\!\! \iint_{\tilde{\Omega} \times \tilde{\Omega}} [\varphi(x) \thetaep(x) - \varphi(y) \thetaep(y)][\Psi(x) - \Psi(y)] K_1 
\\ & \hskip2cm - \frac{1}{2} \int_{\tilde{S}}^{\tilde{T}}\!\!\!\! \iint_{\tilde{\Omega} \times \tilde{\Omega}} [\thetaep(x)^2 - \thetaep(y)^2][\varphi(x)-\varphi(y)] K_1 + \eps \iint \thetaep \Laplace \thetae.
\end{aligned}
\end{equation}
...
On $[\tilde{S}, \tilde{T}]$ we have a uniform $L^\infty(L^\infty)$ bound for $\thetae$. Therefore $\thetaep$ converges in $L^4(L^4)$, and so $\iint \thetaep^2 \ue\cdot\grad\varphi$ is conserved in the limit $\eps \to 0$.  

Since $\Psi$ is smooth by assumption, $\iint \varphi \thetaep \paren{\del_t \Psi + \ue\cdot\grad\Psi}$ is $L^2(L^2)$ continuous and is conserved in the limit $\eps \to 0$.  

To show continuity of the $\iint [\varphi \thetaep - \varphi \thetaep][\Psi-\Psi] K_1$ term, let $f$ be an arbitrary $\HD^{1/4}$ function.  Then
\[ \iint [\varphi(x) f(x) - \varphi(y) f(y)] [\Psi(x) - \Psi(y)] K_1(x,y) \,dxdy \hskip7cm\]
\[\begin{aligned}
\hskip3cm &\leq \paren{\iint [\varphi(x) f(x) - \varphi(y) f(y)]^2 K_{1/2}(x,y)}^{1/2} \paren{\iint [\Psi(x) - \Psi(y)]^2 \frac{K_1(x,y)^2}{K_{1/2}(x,y)} }^{1/2}
\\ &\leq \norm{\varphi f}_{\HD^{1/4}} \paren{\iint [\Psi(x) - \Psi(y)]^2 \frac{K_1(x,y)^2}{K_{1/2}(x,y)} }^{1/2}
\\ &\leq C \norm{f}_{\HD^{1/4}} \norm{\varphi}_{C^1} \norm{\grad \Psi}_{L^\infty} \iint |x-y|^2 |x-y|^{-1/2} \frac{1}{|x-y|^3}.
\end{aligned} \]
In this last line we used Proposition~\ref{thm:Caff Stinga representation} and Lemma~\ref{thm:Lambda stuff} part \eqref{thm:extra product rule}.  It follows that the $\iint [\varphi \thetaep - \varphi \thetaep][\Psi-\Psi] K_1$ term of \eqref{local for eps positive} is continuous under $L^1(\HD^{1/4})$ limits of $\thetaep$.  
Since $\thetae$ are convergent in $L^2(L^2)$ and bounded in $L^2(\HD^{1/2})$, this term is conserved in the limit $\eps \to 0$.  

To show continuity of the $\iint [\thetaep^2 - \thetaep^2][\varphi-\varphi] K_1$ term, let $f$ be an arbitrary $\HD^{1/4}$ function.  Then
\[ \begin{aligned}
\iint [f(x)^2 - f(y)^2][\varphi(x)-\varphi(y)] K_1
&\leq \paren{\iint [f(x)^2 - f(y)^2]^2 K_{1/2}}^{1/2} \paren{ \iint [\varphi(x)-\varphi(y)]^2 \frac{K_1^2}{K_{1/2}} }^{1/2}
\\ &\leq \paren{ 4\!\! \iint\!\! f(x)^2 [f(x)-f(y)]^2 K_{1/2} }^{1/2} \!\! \norm{\grad\varphi}_{L^\infty} \paren{ \iint\!\! \frac{1}{|x-y|^{1.5}} }^{1/2}
\\ &\leq C \norm{f}_{L^\infty} \norm{f}_{\HD^{1/4}}.
\end{aligned}\]
Since all of the terms of \eqref{local for eps positive} behave well in the limit as $\eps \to 0$, it follows that $\theta$ satisfies \eqref{local condition}.  
\end{proof}

%The following lemma is used in the proof of Proposition~\ref{thm:existence}.  
%
%\begin{lemma} \label{thm:continuity of Lambda Psi term}
%For $\Omega \subseteq \R^2$ an open, bounded domain with $C^{2,\beta}$ boundary, $\beta \in {0,1}$ and $\varphi \in \Ctest(\Omega)$ supported on $K \subset\subset \Omega$, and $\Psi \in C^1(\Omega)$, the linear function
%\[ f \mapsto \int \varphi f \Lambda\Psi \]
%is continuous on $\HD^{1/4}$, with constant of continuity depending on $\Omega$, $\norm{\grad\Psi}_{L^\infty(\Omega)}$, $\norm{\varphi}_{C^1(\Omega)}$, and $K$.  
%\end{lemma}
%\begin{proof}
%Break up the expression $\int \varphi f \Lambda\Psi$ into an interior term and a boundary term according to Proposition~\ref{thm:Caff Stinga representation}.  
%
%The interior term is bounded
%\[ \iint [\varphi(x) f(x) - \varphi(y) f(y)] [\Psi(x) - \Psi(y)] K_1(x,y) \,dxdy \hskip7cm\]
%\[\begin{aligned}
%\hskip3cm &\leq \paren{\iint [\varphi(x) f(x) - \varphi(y) f(y)]^2 K_{1/2}(x,y)}^{1/2} \paren{\iint [\Psi(x) - \Psi(y)]^2 \frac{K_1(x,y)^2}{K_{1/2}(x,y)} }^{1/2}
%\\ &\leq \norm{\varphi f}_{\HD^{1/4}} \paren{\iint [\Psi(x) - \Psi(y)]^2 \frac{K_1(x,y)^2}{K_{1/2}(x,y)} }^{1/2}
%\end{aligned} \]
%From Proposition~\ref{thm:Caff Stinga representation}, $K_1 / K_{1/2} \leq C |x-y|^{-1/2}$ for a constant $C$ depending on $\Omega$.  Therefore, since $\Psi$ is Lipschitz and $\Omega$ bounded,
%\[ \iint [\Psi(x) - \Psi(y)]^2 \frac{K_1(x,y)^2}{K_{1/2}(x,y)} \leq C \iint [\Psi(x) - \Psi(y)]^2 |x-y|^{-3.5} \leq C \iint |x-y|^{-1.5} \leq C. \]
%
%For the boundary term, note that $B_1$ is uniformly bounded on any compact subset of $\Omega$ (this follows from the formula for $B_1$ given in \cite{CaSt}, explicit facts about the heat equation on a ball, and the comparison principle).  Therefore, since $\varphi$ compactly supported,
%\[ \int \varphi f \Psi B_1 \leq C \norm{f}_{L^1(\Omega)} \leq C \norm{f}_{\HD^{1/4}}. \]
%
%By Lemma~\ref{thm:Lambda stuff} part \eqref{thm:product rule}, $\norm{\varphi f}_{\HD^{1/4}} \leq C \norm{f}_{\HD^{1/4}}$.  
%
%The result follows.  
%\end{proof}
...
%
%Our goal in this section is to show that the $L^\infty$ norm of solutions to \eqref{eq:main linear} are instantaneously bounded by the $L^2$ norm of the initial data.  Note that global $L^\infty$ bounds for $L^\infty$ initial data are proven in \cite{CoIg.sqg}, and $L^2 \to L^\infty$ regularization holds by construction for the solutions constructed in Proposition~\ref{thm:existence}.  However, we will show the result for arbitrary suitable solutions (in practice, any sufficiently smooth solution).  
%
...
%%\vskip0.3cm
%%
%%Let $\theta$ and $u$ be a suitable solution to \eqref{eq:main linear}.  Then the function
%%\[ \theta_+ := \paren{\theta - \Psi}_+ \]
%%satisfies the inequlality
...
%%\[ [\Psi(x)-\Psi(y)]^2 K(x,y) \leq C |x-y|^{-1} \wedge |x-y|^{2\gamma-3}. \]
...
%%\[ \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \theta_- \leq C \paren{ \abs{\int \theta_+ (\del_t\Psi+u\cdot\grad\Psi)} + \int \indic{\theta_+>0} }.\]
%%\end{proof}
%
%$L^2 \to L^\infty$ regularization is a straightforward application of the standard De Giorgi iteration argument.  Since we make this argument multiple times in this paper, the details are given in the Appendix, see Lemma~\ref{thm:DG1 skeleton}.  
%
%\begin{proposition}[$L^2$ to $L^\infty$] \label{thm:L2 to Linfty}
%Let $T \in \R^+$ be a positive time.  Then for any $S \in (0,T)$, there exists a constant $C = C(S)$ such that the following holds:
%\vskip.3cm
%
%Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\theta$ and $u \in L^\infty(0,T; L^2(\Omega))$ be a suitable solution to \eqref{eq:main linear} on  $\theta_0 \in L^2(\Omega)$.  
%
%Then 
%\[ \norm{\theta}_{L^\infty([S,T]\times \Omega)} \leq C \norm{\theta_0}_{L^2(\Omega)}. \]
%\end{proposition}
%
%Proposition \ref{thm:L2 to Linfty} is an easy consequence of these lemmas.  
%
%\begin{proof}[Proof of \ref{thm:L2 to Linfty}]
%Our regularity assumptions on $\theta$ and $u$ are enough to show, by the usual energy argument, that $\norm{\theta(t,\cdot)}_{L^2(\Omega)}$ is decreasing in time.  By the main result of \cite{CoIg.sqg}, or by applying \eqref{leray-hopf condition} with constant cutoff $\Psi(t,x) = \norm{\theta(T,\cdot)}_{L^\infty(\Omega)}$, the $L^\infty$ norm of $\theta(t,\cdot)$ is also decreasing in time once finite.  Thus for $T$ large, $\norm{\theta}_{L^\infty([2,T]\times \Omega)} \leq \norm{\theta}_{L^\infty([1,2]\times\Omega)}$.  
%\vskip.3cm
%
%In the case $S \geq 2$, we can apply \eqref{leray-hopf condition} to establish the family of energy inequalities assumed in Lemma~\ref{thm:DG1 skeleton} and then apply Lemma~\ref{thm:DG1 skeleton} to the function $\frac{\sqrt{\delta}}{\sqrt{2} \norm{\theta_0}_2} \theta(t+2,x)$ to show that 
%\[ \norm{\theta}_{L^\infty([2,T]\times\Omega)} \leq \paren{\frac{2}{\delta}}^{1/2} \norm{\theta_0}_2. \]
...
%\[ \norm{\theta}_{L^\infty([S,T]\times\Omega)} \leq \frac{C}{S^{1/2}} \norm{\theta_0}_{L^2(\Omega)}. \]
...
To that end, we shall consider, for any domain $\Omega$ and time $T$, functions $\theta:[-T,0]\times\Omega \to \R$, $\ulow$ and $\uhigh:[-T,0]\times \Omega \to \R^2$, and a Lipschitz path $\Gammal:[-T,0] \to \Omega$ which satisfy
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\Gamma:[-T,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} hold on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{3/4} K_{1}(x,y). \]

Let $\theta$, $\ulow$, $\uhigh$, $\Gammal$ solve \eqref{eq:main linear brokendown} on $[-T,0]\times\Omega$ with $\theta$ and $\ulow+\uhigh$ a suitable pair, and satisfy $\norm{\Lambda^{-1/4} \uhigh}_{L^\infty([-T,0]\times\Omega)} \leq 6 \Ccalib$, and $\norm{\grad \ulow}_{L^\infty([-T,0]\times\Omega)} \leq 2\Ccalib$.   Denote $\gamma := \Gammal - \gamma$ and assume $\norm{\dot{\gamma}}_{L^\infty([-T,0])} \leq \Cgamma$ and $\gamma(0) = 0$.  
\vskip.3cm
...
\[ \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \theta_- \leq C \paren{ \int \indic{\theta_+ \geq 0} + \int \theta_+ + \int \theta_+^2 }. \]
...
Applying \eqref{leray-hopf condition} to $\theta$ and $\Psi$ we arrive at
\begin{equation} \label{control from cacciopolli} \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \theta_- \leq C \paren{ \int \indic{\theta_+ \geq 0} + \abs{\int \theta_+ (\ulow-\dot{\Gamma}(t) + \uhigh) \cdot \grad\psi(x-\Gamma(t))} }. \end{equation}
\vskip.3cm
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\Gamma:[-2,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} holds on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{3/4} K_{1}(x,y). \]

Let $\theta$, $\ulow$, $\uhigh$, and $\Gammal$ solve \eqref{eq:main linear brokendown} on $[-2,0]\times\Omega$ with $\theta$ and $\ulow+\uhigh$ a suitable pair, and satisfy $\norm{\Lambda^{-1/4} \uhigh}_{L^\infty([-2,0]\times\Omega)} \leq 6 \Ccalib$, and $\norm{\grad \ulow}_{L^\infty([-2,0]\times\Omega)} \leq 2\Ccalib$.  Denote $\gamma := \Gammal-\Gamma$ and assume $\norm{\dot{\gamma}}_{L^\infty([-2,0])} \leq \Cgamma$ and $\gamma(0) = 0$.  
\vskip.3cm
...
and obtain
\[ \ddt \int \theta_a^2 + \int \abs{\Lambda^{1/2} \theta_a}^2 \leq C \paren{ \int \indic{\theta_a \geq 0} + \int \theta_a + \int \theta_a^2 }. \]
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$ , and let $\Gamma:[-5,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} holds on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{3/4} K_{1}(x,y). \]

Let $\theta$, $\ulow$, $\uhigh$, and $\Gammal$ solve \eqref{eq:main linear brokendown} on $[-5,0]\times\Omega$ with $\theta$ and $\ulow+\uhigh$ a suitable pair, and satisfy $\norm{\Lambda^{-1/4} \uhigh}_{L^\infty([-5,0]\times\Omega)} \leq 6 \Ccalib$, and $\norm{\grad \ulow}_{L^\infty([-5,0]\times\Omega)} \leq 2\Ccalib$.  Denote $\gamma := \Gammal - \Gamma$ and assume $\norm{\dot{\gamma}}_{L^\infty([-5,0])} \leq \Cgamma$ and $\gamma(0) = 0$.  
\vskip.3cm
...
Our goal is to bound the derivatives of $\theta_+^2$ so that we can apply a compactness argument to the sequence $\theta_n$.  (For the curious reader, it is the calculations in Step 2 below in which it becomes necessary to consider $\theta_+^2$ instead of $\theta_+$.)  
...
Since each $\theta_n$ and $\uhigh^n + \ulow^n$ is a suitable pair by assumption, we can apply the inequality \eqref{local condition} to find that, for some constant $C$ independent of $n$ and of $\varphi$, on $[-4,0]\times \Omega_n$
\begin{equation}\label{time compactness setup}\begin{aligned} 
...
\\ & - \iint \theta_+^2 \Lambda\varphi 
- 2 \int \iint [\varphi(x) \theta_+(x) - \varphi(y) \theta_+(y)][\psi(x-\Gamma_n) - \psi(y-\Gamma_n)] K_1. 
\end{aligned}\end{equation}
...
%\int \uhigh^n \cdot (\theta_+ \grad\psi) &\leq C, \\
\iint \uhigh^n \cdot (\theta_+ \varphi\grad\psi) &\leq C \paren{\norm{\varphi}_{C^0(-4,0;L^\infty(\Omega))} + \norm{\varphi}_{C^0(-4,0;C^1(\Omega))}}.
\end{aligned} \end{equation}

Since $\norm{\Lambda \varphi}_{L^2} = \norm{\grad \varphi}_{L^2}$, we have
\begin{equation} \label{Lambda phi bounds}
- \iint \theta_+^2 \Lambda \varphi \leq \norm{\varphi}
\end{equation}

Plugging these six bounds into \eqref{time compactness setup}, we have for a constant $C$ independent of $n$ and $\varphi$
\begin{equation} \label{theta3 compact in time} \int_{-4}^0 \int_{\Omega_n} \paren{\del_t \theta_+^2 + \dot{\Gamma}_n\cdot\grad\theta_+^2}\varphi \,dxdt \leq C \norm{\varphi}_{C^0(0,T; C^2(\Omega))}. \end{equation}

\vskip.3cm
\step{Taking the limit}

We wish to analyze the limiting behavior of $\theta_+^2$ in the vicinity of $\Gamma_n$.  First we shift these functions to remove the dependence on $\Gamma_n$, and
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\Gammal:[-5,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} hold on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{3/4} K_{1}. \]
...
Suppose that $\theta:[-T,0] \times \Omega \to \R$ and $u:[-T,0]\times \Omega \to \R^2$ solve \eqref{eq:main linear} and $u$ is calibrated by a sequence $(u_j)_{j \geq j_0}$ with constant $\Ccalib$ and center $N$.  
% satisfies
%\[ u = \sum_{j=j_0}^\infty u_j \]
%with that sum converging in $L^2(\Omega)$ and $(u_j)_j$ calibrated with constant $\Ccalib$ and center $N$.  

Suppose that on $\Omega$ the functions $K_{1/4}$ and $K_1$ (defined in Proposition~\ref{thm:Caff Stinga representation}) satisfy the relation
...
also solve \eqref{eq:main linear} on $[-T/\eps, 0]\times \Omega_\eps$ where $\Omega_\eps = \{x \in \R^2: \eps x \in \Omega\}$.  
...
for a sequence $(u_j)_{j\geq j_0}$ of divergence-free functions calibrated with some constant $\Ccalib = \Ccalib(\Omega)$ and center 0.  Assume without loss of generality that $j_0 < 0$.
...
\end{align*}
The quantity $\gamma_k$ here corresponds to the part of the drift coming from frequency packets $u_j$ which are part of the definition of $\ulowth{k}$ but are not contained in $\ulowth{k-1}$ (they would instead be included in $\uhighth{k-1}$).  

Since each $\ulowth{k}$ is $L^\infty$ in space-time and Lipschitz in space, these $\gamma_k$ exist by a version of the Cauchy-Lipschitz theorem.  For example, Theorem 3.7 of Bahouri, Chemin, and Danchin \cite{BaChDa} proves existence and uniqueness in our case.  In particular, since $\ulowth{k}$ is a vector field which is tangential to the boundary of $\Omega_k$ and has unique flows, the path $\Gamma_k + \gamma_k$ which follows this vector field must remain inside $\bar{\Omega}_k$ for all time and so our expressions remain well-defined.  
\vskip.3cm
...
From Lemma \ref{thm:scaling}, we know that $\theta_k$ and the calibrated function $\sum_{j \geq j_0} u_j(\eps^k \cdot, \eps^k \cdot)$ solve \eqref{eq:main linear}.  By construction, $\theta_k$, $\ulowth{k}$, $\uhighth{k}$, $\Gamma_k$, and $\gamma_k$ solve \eqref{eq:main linear brokendown}
\vskip.3cm
...
This completes the proof of regularity.  That $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; H_0^1(\Omega))$ implies $\theta$, $u$ are a suitable pair is proven in Lemma~\ref{thm:caccioppoli}.  
