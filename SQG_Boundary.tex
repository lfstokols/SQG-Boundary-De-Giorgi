...
In this paper, we consider critical SQG on a bounded domain.  For $\Omega$ a bounded domain in $\R^2$ with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, consider the Laplacian with homogeneous Dirichlet boundary conditions $(-\Laplace_D)$ and 

In \cite{CoIg.fraclap} and \cite{CoIg.fraclap}, Constantin and Ignatova consider the version of SQG on bounded domains.  For $\Omega$ a bounded domain in $\R^2$ with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, they consider the Laplacian with homogeneous Dirichlet boundary conditions $(-\Laplace_D)$ and 
...
In \cite{CoIg.fraclap} existence is shown.  In \cite{CoIg.sqg}, it is proven that smooth solutions to \eqref{eq:main nonlinear} are regular in the interior of $\Omega$.  The method of proof is by nonlinear maximum principles [give details].  However, these formulae blow up near the boundary and so the method seems inapplicable to the question of global regularity.  
...
The existence of a weak solution (meaning solution in the sense of distributions) $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; \HD^{1/2}(\Omega))$ is proven in \cite{CoIg.fraclap}.  
