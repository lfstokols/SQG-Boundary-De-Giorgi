...
Let $\theta:[-5,0] \times \Omega \to \R$ and $u:[-5,0]\times\Omega \to \R^2$ be a suitable solution to \eqref{eq:main linear}.  Assume that $u$ is the Riesz transform of an $L^\infty$ function, i.e. $\norm{\Lambda \grad^\perp \cdot u}_{L^\infty([-5,0] \times \Omega)} \leq k$.  
