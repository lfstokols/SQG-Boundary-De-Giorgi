...
Our result will apply to solutions which verify a certain a priori estimate which holds, in particular, for sufficiently smooth solutions. We begin with a proposition:
\begin{proposition}[A Priori Estimate] \label{thm:suitability}
There exists a universal constant $\Csuit$ such that the following holds:

Let $\Omega \subseteq \R^2$ be bounded and open with $C^{2,\beta}$ boundary, $\beta \in (0,1)$, and let $T > 0$ be a time.  Let $\theta, u$ be a solution to \eqref{eq:main nonlinear} on $\Omega \times [0,T]$, with $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; H_0^1(\Omega))$.  
...
We will consider solutions which verify the following technical condition:
\begin{definition}
A pair $\theta, u \in L^\infty(0,T;L^2(\Omega))$ is called \textbf{suitable} if it satisfies \eqref{leray-hopf condition} and \eqref{local condition} at all scales with the same universal constant $\Csuit$ defined in Proposition~\ref{thm:suitability}.  Specifically, if the following holds:

Let $\lambda >0$ and $\eps \in (0,1)$ be given and let $\Psi \in C^\infty([0,\infty)\times \R^2)$ be any smooth non-negative function satisfying $\norm{\grad\Psi}_{L^\infty([0,\infty)\times\R^2)} \leq k$ and $\sup_{[0,\infty)} \bracket{\Psi(t,\cdot)}_{1/4; \R^2} \leq k$ for some constant $k$.  

Define $\tilde{\Omega} := \{x \in \R^2: \eps x \in \Omega\}$, $\tilde{\theta}_+(t,x) := (\lambda \theta(\eps t, \eps x)-\Psi(t, x))_+$, and $\tilde{u}(t,x):= u(\eps t, \eps x)$.  Let $\varphi \in C^\infty_0(0,T;C^\infty(\tilde{\Omega}))$ be non-negative.  

Then $\tilde{\theta}_+$ and $\tilde{u}$ and $\varphi$ and $\Psi$ satisfy \eqref{leray-hopf condition} and \eqref{local condition} on $[0,\eps\n T] \times \tilde{\Omega}$ with the universal constant $\Csuit$.  
\end{definition}
Suitability is considered because it is stable under weak limits, as we will show in Section~\ref{sec:suitable}.  We will show that this property holds for sufficiently smooth solutions to \eqref{eq:main nonlinear}.  In \cite{CoNg.strong}, Constantin and Nguyen demonstrate local-in-time existence for suitable solutions to \eqref{eq:main nonlinear}, assuming initial data in $H^2(\Omega) \cap H_0^1(\Omega)$.  In this paper, we will show that suitable solutions, when they exist, are H\"{o}lder continuous uniformly with respect to the $L^2$ norm of the initial data.  

The main result of this paper is the following:
\begin{theorem} \label{thm:main continuity}
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary, $\beta \in (0,1)$, let $t > 0$ be a time, and let $K > 0$ any constant.  Then there exists a constant $C > 0$ depending on $\Omega$ and $t$, and a constant $\alpha \in (0,1)$ depending on $\Omega$, $t$, and $K$, such that the following holds:
\vskip.3cm
...
%\vskip.3cm
% with $\theta \in L^\infty(0,t; L^2(\Omega) \cap H_0^1(\Omega) \cap L^4(\Omega))$ and $u \in L^\infty(0,t; L^4(\Omega))$ is proven for initial data sufficiently smooth.  Therefore, going forward, we can assume this much regularity on our solutions.  

The Paper is organized as follows. Section \ref{sec:lambda} is dedicated to basic properties of the operator $\Lambda$ and the corresponding Sobolev spaces $\HD^s$.  In Section~\ref{sec:suitable} we prove Proposition~\ref{thm:suitability} and discuss suitable solutions.  In Section~\ref{sec:Linfty} we prove $L^2$ to $L^\infty$ regularization.  In Section~\ref{sec:littlewood paley} we prove that the Riesz transform of the $L^\infty$ function $\theta$ is callibrated.  Section~\ref{sec:de giorgi} contains the De Giorgi Lemmas.  Section~\ref{sec:harnack} is dedicated to the local decrease in oscillation through an analog of the Harnack inequality.  Finally in Section~\ref{sec:holder} we prove the main theorem, Theorem~\ref{thm:main continuity}.  In the Appendix \ref{sec:technical} we prove a few technical lemmas which are needed in the main paper.  
\vskip.3cm
...
%-------%-------%-------%-------%-------%-------%-------%-------%-------%-------%-------%-------
\vskip1cm
\section{Suitable solutions} \label{sec:suitable}

In this section, we will prove Proposition \ref{thm:suitability}, and show that sufficiently regular weak solutions of \eqref{eq:main linear} are suitable.  
...
%\vskip0.3cm
%
%Let $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T;H_0^1(\Omega))$ and $u \in L^\infty(0,T; L^2(\Omega))$ solve \eqref{eq:main linear} in the sense of distributions.  Then the functions
...
%
%In addition, for any non-negative function $\varphi \in C^0(0,T;C^1(\Omega))$,
%\begin{equation} \label{second energy inequality}\begin{aligned}
%\frac{1}{2} \iint \varphi \del_t \theta_+^2 &\leq \frac{1}{2} \iint \theta_+^2 u\cdot\grad\varphi
%+ C\abs{\iint \varphi \theta_+ \del_t\Psi + u\cdot\grad\Psi}
%\\ & + C\norm{\varphi}_{C^0(0,T;C^1(\Omega))} \paren{\abs{\iint \theta_+ \del_t\Psi + u\cdot\grad\Psi} 
%+ \sup_{[0,T]} \int \theta_+^2 + \int_0^T |\{\theta_+ \geq 0\}|^2 + 1} 
%\end{aligned}\end{equation}
%
%Moreover, $\theta$ and $u$ are a suitable pair.
%\end{lemma}
...
\frac{1}{2} \int \varphi \del_t \theta_+^2 - \int \varphi \theta_+ \Lambda \theta_- = \frac{1}{2} \int  \theta_+^2 u \cdot \grad \varphi - \int \varphi \theta_+ \paren{\del_t \Psi + u \cdot \grad \Psi} + - \int \varphi \theta_+ \Lambda \Psi - \int \varphi \theta_+ \Lambda \theta_+. 
\end{equation}
Note that the $\Lambda \theta_-$ term on the left-hand side of this equality is non-negative by Lemma~\ref{thm:Lambda stuff} part \eqref{thm:disjoint}.  It remains to bound, on the right-hand-side, the $\Lambda \Psi$ and $\Lambda \theta_+$ terms.  

For the $\Lambda \theta_+$ term, we can bound
\[ - \int \varphi \theta_+ \Lambda \theta_+ \leq C \norm{\varphi\theta_+}_{\HD^{1/2}} \norm{\theta_+}_{\HD^{1/2}}. \]
Moreover, just as argued above for \eqref{good lower bound on psi term cacciopoli}, we can bound
\[ - \int \varphi \theta_+ \Lambda \Psi \leq C k^2 \int \indic{\theta_+>0} \,dx \norm{\varphi \theta_+}_{\HD^{1/2}}. \]
...
\begin{lemma}
Any function is a suitable solution.
\end{lemma}
\begin{proof}
Finally, to see that $\theta$, $u$ is a suitable pair, we need only notice that for any $\lambda > 0$, $\eps \in (0,1)$ our assumptions on $\theta$ and $u$ are preserved by the scaling $\theta(t,x) \mapsto \lambda \theta(\eps t, \eps x)$, $u(t,x) \mapsto u(\eps t, \eps x)$, and so the above proof holds at any scale.  
\end{proof}

\begin{proposition}[Stability of suitable solutions]
If $\theta_n$, $u_n$ is a sequence of suitable pairs such that $u_n = R^\perp \theta_n$, and $\theta_n \weakly \theta$ converges in weak $L^2$, then $u_n \to u := R^\perp \theta$ in $L^2$ and $\theta$, $u$ is a suitable pair.  
\end{proposition}
\begin{proof}
Since $R^\perp$ is continuous on $L^2$, we have $u_n \to u$ weakly in $L^2$.  Moreover, since weakly convergent sequences are bounded, we know $\theta_n$ and $u_n$ are bounded in $L^2$.  

Choose $\psi$, $\eps$, $varphi$.  For each $n$, we can apply \eqref{leray-hopf condition} and \eqref{local condition} to $\tilde{\theta_n}$ and $\tilde{u_n}$ to see that $\norm{\tilde{\theta}_+}_{\HD^{1/2}}$ is a bounded sequence and $\del_t \tilde{\theta}_+^2$ is bounded in $\mathcal{M}(0,T; C^{-2}(\Omega))$.  Therefore by Aubin-Lions, $(\theta_n)_+ \to \theta_+$ strongly in $L^2$.  By taking $\psi \equiv -k$ for all $k \in \N$, we can say further that $\theta_n \to \theta$ and so, again since $R^\perp$ is continuous on $L^2$, $u_n \to u$ strongly in $L^2$.  

Choose $\psi$, $\eps$, and $\varphi$, and apply \eqref{leray-hopf condition} and \eqref{local condition} to $\tilde{\theta_n}$ and $\tilde{u_n}$.  The right-hand side of these inequalities will limit to the same expression for $\tilde{\theta}$, $\tilde{u}$.  The left-hand side of \eqref{leray-hopf condition} is convex so its limit will be greater than the same expression applied to $\theta$.  The left-hand side of \eqref{local condition} is also continuous on $L^2$, so its limit will equal the same expression for $\tilde{\theta}$.  
\end{proof}

Given a parameter $\eps$, consider solutions $\theta_\eps$ to 
\[ \del_t \theta_\eps + u_\eps \cdot \grad \theta_\eps + \Lambda \theta_\eps = \eps \Laplace \theta_\eps. \]
\[ u_\eps = \grad^{\perp} \Lambda\n \theta_\eps. \]
%As a variational formulation, let $\varphi$ be any $H_0^1$ function.  
%\[ \int \del_t \theta_\eps \varphi - \int u_\eps \theta_\eps \grad \varphi + \int \theta_\eps \Lambda \varphi + \eps \int \grad \theta_\eps \cdot \grad \varphi = 0. \]
%\[ \int \del_t \theta_\eps \varphi + B_t(\theta_\eps, \varphi) = 0. \]
%
%Note that $B_t(\theta_\eps, \varphi)$ is linear in $\varphi$ but not in $\theta_\eps$, and 
%\[ B_t(\theta_\eps, \varphi) \leq \norm{\theta_\eps}_{H^1}^2 \norm{\varphi}_{H^1} + \norm{\theta_\eps}_2 \norm{\varphi}_{H^1} + \norm{\theta_\eps}_{H^1} \norm{\varphi}_{H^1}. \]

Well anyways, if $\theta_\eps$ exists, then they are uniformly bounded in $L^2(H^{1/2})$ and $L^\infty(L^2)$, so they have a strong $L^2$ limit and so do $u_\eps$.  Moreover, $L_\eps(\theta_\eps) \to L(\theta_0)$ so $\theta_0$ solves the original thing.  Moreover, $\theta_0$ is suitable.  

%For existence, let's try a fixed point thing.  Let $F:L^2(H_0^1)$ defined by
%\[ F(\theta)(t) = \theta_0 + \int_0^t \bracket{u \cdot \grad \theta + \Lambda \theta - \eps \Laplace \theta} \,ds. \]
%That integrand is less that
%\[ \norm{u \cdot \grad \theta + \Lambda \theta - \eps \Laplace \theta}_{H_0^1} \]
%
%Define $F(\theta)$ the solution to
%\[ \del_t v - \eps \Laplace v = -\bracket{u \cdot \grad \theta + \Lambda \theta}. \]
%Note that, for $\theta \in L^2(H^1)$, the right-hand side is in $L^1(L^{4/3}) + L^2(L^2)$.  
%
%If the rhs is in $L^2(H^{-1})$, then $v \in L^2(H^1)$.  
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\theta$ and $u \in L^\infty(0,T; L^2(\Omega))$ be a suitable pair solving \eqref{eq:main linear} in the sense of distributions with $\theta_0 \in L^2(\Omega)$.  

Then 
\[ \norm{\theta}_{L^\infty([S,T]\times \Omega)} \leq C \norm{\theta_0}_{L^2(\Omega)}. \]
\end{proposition}

In order to prove this proposition, we require an energy inequality for $(\theta-C)_+$ for arbitrary constants $C$, which follows from the suitability condition.  With this energy inequality, Proposition~\ref{thm:L2 to Linfty} follows by a standard De Giorgi argument.  Since we will need to make a similar argument again in Section~\ref{sec:de giorgi}, we will make the bulk of the argument in a more general lemma.  
...
Our goal in this section is to prove De Giorgi's first and second lemmas for solutions to \eqref{eq:main linear} with $u$ uniformly calibrated.  The De Giorgi lemmas will eventually be applied iteratively to various rescalings of the solution $\theta$, so the following results must be independent of the size of the domain $\Omega$.  Any properties we do assume for the domain, such as the regularity of the boundary, must be scaling invariant.  
\vskip.3cm
...
In order to prove the De Giorgi lemmas, we must derive an energy inequality for the function $\paren{\theta - \Psi}_+$ where $\Psi(t,x)$ grows sublinearly in $|x|$.  Considering the suitability condition \eqref{leray-hopf condition}, we see that control can only be gained if the quantity $\del_t \Psi + u \cdot \grad\Psi$ is bounded.  This requires a barrier function which is moving in space along a Lagrangian path of $\ulow$.  That Lagrangian path must further be broken up into two parts, one corresponding to the lowest frequencies and one corresponding to mid-range frequencies.  
\vskip.3cm

To that end, we shall consider, for any domain $\Omega$ and time $T$, functions $\theta:[-T,0]\times\Omega \to \R$, $\ulow$ and $\uhigh:[-T,0]\times \Omega \to \R^2$, and paths $\Gamma$ and $\gamma:[-T,0] \to \R^2$ which satisfy
...
\Gamma(t) + \gamma(t) \in \Omega & \textrm{on } [-T,0], \\
\dot{\Gamma}(t) + \dot{\gamma}(t) = \ulow(t, \gamma(t) + \Gamma(t)) & \textrm{on } [-T,0], \\
\gamma(0) = 0.
\end{cases} \end{equation}

Generally speaking $\ulow$ and $\gamma$ will be locally Lipschitz functions while $\uhigh$ is merely in a weak space $W^{-1/4,\infty}$.  We generally assume no regularity for $\Gamma$, instead we will assume that $\theta$ is bounded in relation to $\Gamma$.  See Section~\ref{sec:holder} for the construction of these functions $\Gamma$ and $\gamma$.  
\vskip.3cm
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$.  Assume that Proposition~\ref{thm:Caff Stinga representation} hold on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{1/2} K_{1}(x,y). \]

Let $\theta$, $\ulow$, $\uhigh$, $\Gamma$ and $\gamma$ solve \eqref{eq:main linear brokendown} on $[-T,0]\times\Omega$ with $\theta$ and $\ulow+\uhigh$ a suitable pair, and satisfy $\norm{\Lambda^{-1/4} \uhigh}_{L^\infty([-T,0]\times\Omega)} \leq 6 \Ccalib$, $\norm{\grad \ulow}_{L^\infty([-T,0]\times\Omega)} \leq 2\Ccalib$, and $\norm{\dot{\gamma}}_{L^\infty([-T,0])} \leq \Cgamma$.  
\vskip.3cm
...
Let $\theta$, $\ulow$, $\uhigh$, $\Gamma$ and $\gamma$ solve \eqref{eq:main linear brokendown} on $[-2,0]\times\Omega$ with $\theta$ and $\ulow+\uhigh$ a suitable pair, and satisfy $\norm{\Lambda^{-1/4} \uhigh}_{L^\infty([-2,0]\times\Omega)} \leq 6 \Ccalib$, $\norm{\grad \ulow}_{L^\infty([-2,0]\times\Omega)} \leq 2\Ccalib$, and $\norm{\dot{\gamma}}_{L^\infty([-2,0])} \leq \Cgamma$.  
\vskip.3cm
...
Let $\theta$, $\ulow$, $\uhigh$, $\Gamma$ and $\gamma$ solve \eqref{eq:main linear brokendown} on $[-5,0]\times\Omega$ with $\theta$ and $\ulow+\uhigh$ a suitable pair, and satisfy $\norm{\Lambda^{-1/4} \uhigh}_{L^\infty([-5,0]\times\Omega)} \leq 6 \Ccalib$, $\norm{\grad \ulow}_{L^\infty([-5,0]\times\Omega)} \leq 2\Ccalib$, and $\norm{\dot{\gamma}}_{L^\infty([-5,0])} \leq \Cgamma$.  
\vskip.3cm
...
Since each $\theta_n$ and $\uhigh^n + \ulow^n$ is a suitable pair by assumption, we can apply the inequality \eqref{local condition} to find that, for some constant $C$ independent of $n$ and of $\varphi$,
\begin{equation}\label{time compactness setup}\begin{aligned} 
\iint \varphi \del_t \theta_+^2 + \iint \varphi \dot{\Gamma}_n\cdot\grad\theta_+^2 &\leq \iint \theta_+^2 (\ulow^n - \dot{\Gamma}_n + \uhigh^n)\cdot\grad\varphi + C \bigg(\norm{\varphi}_{C^0(C^1)} \abs{\iint \theta_+ \paren{\ulow^n - \dot{\Gamma}_n + \uhigh^n}\cdot\grad\psi} 
\\ & \hspace{4cm} + \abs{\iint \varphi \theta_+ \paren{\ulow^n - \dot{\Gamma}_n + \uhigh^n}\cdot\grad\psi} + \norm{\varphi}_{C^0(C^1)}\bigg). 
\end{aligned}\end{equation}
...
For the high pass terms, we have $\uhigh^n$ uniformly bounded in $\dot{W}^{-1/4,\infty}$ so, by Lemma~\ref{thm:Lambda stuff} parts \ref{thm:L1 of Lambda1/4 bounded} and \ref{thm:extra product rule}, there is a constant $C$ independent of $n$ and $\varphi$ such that
\begin{align*}
\int \uhigh^n \cdot (\theta_+ \grad\varphi) &\leq C \paren{\norm{\grad\varphi}_{L^\infty(\Omega)} + \norm{\varphi}_{C^2(\Omega)}}, \\
\int \uhigh^n \cdot (\theta_+ \grad\psi) &\leq C, \\
\int \uhigh^n \cdot (\theta_+ \varphi\grad\psi) &\leq C \paren{\norm{\varphi}_{L^\infty(\Omega)} + \norm{\varphi}_{C^1(\Omega)}}.
\end{align*}
...
The idea of the proof is to consider a small enough time interval that $\Gamma(t) + \gamma(t)$ is very close to $\Gamma(t)$.  This is possible because $\gamma$ is Lipschitz by assumption.  
