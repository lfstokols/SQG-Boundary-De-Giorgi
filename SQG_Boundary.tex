...
\begin{proof}
The first claim is \cite{IMTbilinear} Lemma 3.5, and it is also an immediate corollary of \cite{IMT-schrodinger} Theorem 1.1.  

The second claim follows from \cite{IMTbilinear} Lemma 3.6.  This lemma requires that
...
which, by a trivial modification of the proof in \cite{IMT bilinear}, is enough to prove the result stated here.  
\end{proof}
...
\section{Properties of Calibrated Sequences} \label{sec:calibrated functions}
We've shown in the previous section that our drift term $u$ is a sum of a calibrated sequence $(u_j)_{j \in \Z}$. 
...
\[ u = \ulow + \uhigh \]
with 
\[ \norm{\grad \ulow}_{L^\infty(-T,0)\times \Omega} \leq C_1 \kappa\]
and
\[ \bracket{\ulow}_{3/4} \leq 2 \kappa \]
and
...
\bibliography{SQG-references.bib}
