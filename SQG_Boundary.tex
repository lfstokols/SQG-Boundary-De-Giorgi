...
\begin{proof}
Let $g$ be any Schwarz function in $L^2(\R^2)$, and let $f$ be a function in $H^{s+1}_D$.  Let $E:H^1_D(\Omega) \to H^1(\R^2)$ be a the extension-by-zero operator, where $H^1$ denotes the classical Sobolev space defined using the gradient.  Define the function
...
It remains to ask whether $\Lambda^{s-z} f$ is in $H^1_D$ so that we can apply Lemma~\ref{thm:H1 and H1}.  However, this is true based on our assumption $f \in H^{1+s}_D$, since the various powers of $\Lambda$ all commute and form a semigroup.  Ergo
...
It follows that for any Schwarz function $g \in L^2(\R^n)$ and any $f \in H_D^{s+1}$,
...
Thus
\[ \HDh{(f-g)_+} \leq 2\iint [f(x)-f(y)]^2 K + 2\iint [g(x)-g(y)]^2 K + \int f^2 B = 2\iint [g(x)-g(y)]^2 K + 2\HD{s}{f}. \]
...
\[ \abs{\Rom{1}} \leq 2 \iint \indic{f \neq 0}(x) \abs{g(x)-g(y)} \cdot \abs{f(x)-f(y)} \cdot |x-y|^s K_{2s}. \]
...
Suppose that $\Gamma, \gamma \in \Lip([-T,0])$ satisfy $\norm{\dot{\gamma}}_\infty \leq \Cgamma$, $\gamma(0)=0$, and
...
for some $\gamma$ with Lipschitz norm less than $\Cgamma$.  
...
However, by Sobolev embedding and the fact that $H_D^{1/2}$ controls classical $H^{1/2}$ controls $L^4$, we know from Reisz-Thorin that the left side of the energy inequality controls the $L^3$ norm of $\theta_{k+1}$ so
\[ \norm{\theta_{k+1}}_{L^3([-1-2^{-k-1},0]\times\Omega)} \leq C^k \E_k^{1/2}. \]
