...
\end{cases}
\end{equation}
uniformly with respect to $\eps \geq 0$.  
...
\eps \int \theta_+ \Laplace (\theta_+ + \Psi) &= -\eps \int \varphi \grad \theta_+ \cdot \grad (\theta_+ + \Psi) - \eps \int \theta_+ \grad \varphi \cdot \grad (\theta_+ + \Psi)
...
We will construct solutions to \eqref{eq:main linear} (equivalently \eqref{eq:viscous linear} with $\eps = 0$) which verify these energy inequalities proven above but which are not a priori in $L^2(H_0^1)$.  
\begin{definition}
A pair $\theta, u$ is called a \textbf{suitable solution} to \eqref{eq:main linear} on a space time domain $[0,T]\times \Omega$ if $T <\infty$, $\Omega \subseteq\R^2$ open and bounded,  $\theta,u \in L^\infty(0,T;L^2(\Omega)$, $\theta \in L^2(0,T;\HD^{1/2}(\Omega))$, $u \in L^4(0,T; L^4(\Omega))$ and
\begin{enumerate}
...
In particular, $\theta \in C^0([0,\infty);L^2(\Omega)) \cap L^2([0,\infty);H_0^1(\Omega))$ and $\del_t \in L^2([0,\infty);H^{-1}(\Omega))$, and $\theta(t,\cdot) \to \theta_0(\cdot)$ weakly in $L^2(\Omega)$ as $t \to 0$, and for any $S > 0$
...
Note that $\thetae$ and $\ue$ are weak solutions to \eqref{eq:viscous linear}.  They therefore satisfy \eqref{leray-hopf condition}.  The terms $\int \thetaep (\del_t\Psi+\ue\cdot\grad\Psi)$ and $\int \indic{\thetaep\geq 0}$ are continuous under $L^2$ limits, and the quantities $\ddt \int \thetaep^2$ and $\int \abs{\Lambda^{1/2} \thetaep}^2$ are lower-semicontinuous under $L^2$ limits, so we conclude that $\theta$ and $u$ satisfy \eqref{leray-hopf condition}.  

Similarly, $\thetae$ and $\ue$ satisfy \eqref{local condition}.  
...
The remaining terms in \eqref{local condition} are $L^2(L^2)$ continuous, so $\theta$ and $u$ satisfy \eqref{local condition}.  
...
Consider a point $(t_0, x_0)$ with $t_0 > S$.  Consider arbitrary constants $\lambda$ and $\eps$ and note that
\[ \tilde{\theta}(t,x) := \lambda \theta(\mu (t_0 + t), \mu x), \qquad \tilde{u}(t,x) := u(\mu (t-t_0), \mu x) \]
is a suitable solution to \eqref{eq:main linear}.  If $\mu \geq (t_0 - S)\n$, then for $\tilde{\Omega} := \{x\in \R^2: \mu x \in \Omega \}$, we have 
\[ \norm{\tilde{\theta}}_{L^\infty([-5,0]\times\tilde{\Omega}} \leq C_1 k. \]
If moreovere $\lambda C_1 k \leq 2$ then we can apply Lemma~\ref{thm:continuity for suitable} to $\tilde{\theta}$, $\tilde{u}$ with
\[ \norm{\Lambda^{-1} \grad^\perp \cdot \tilde{u}}_{L^\infty([-5,0]\times\Omega)} \leq C_1 k \]
and find that $\tilde{\theta}$ is H\"{o}lder continuous at $(t_0,x_0)$ with constant $\alpha$ depending on $k$ and $S$.  
