...
5 \max\paren{ - \kappa \ln_2(\eps) e^{10\eps\kappa}, j_0 \kappa} \leq \eps\n - 3. 
\end{equation}
...
From Lemma \ref{thm:scaling}, we know that $\theta_k$ and the calibrated sequence $(u_j(\eps^k \cdot, \eps^k \cdot))_j$ solve \eqref{eq:main linear}.  By Lemma~\ref{thm:calibration is good}, we know that independently of $k$
...
Once I get this trimmed down to be readable, I should be able to apply the Harnack inequality to each $\theta_k$ which proves the result.  The only concern is that when I have to flip it upside down, have I proved that the bound from below is maintained?  Surely I was supposed to add something, or else I'm proving that the function is 0 at the point in question.  If I never have to flip, that would be true.  Oh lord I really missed something.  
