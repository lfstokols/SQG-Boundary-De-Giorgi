...
\frac{1}{2} \int \varphi \del_t \theta_+^2 - \int \varphi \theta_+ \Lambda \theta_- + \int \varphi \theta_+ \Lambda \theta_+ + \int \varphi \theta_+ \Lambda \Psi = \frac{1}{2} \int  \theta_+^2 u \cdot \grad \varphi - \int \varphi \theta_+ \paren{\del_t \Psi + u \cdot \grad \Psi} +. 
\end{equation}
Note that the $\Lambda \theta_-$ term on the left-hand side of this equality is non-negative by Lemma~\ref{thm:Lambda stuff} part \eqref{thm:disjoint}.  It remains to bound, on the left-hand-side, the $\Lambda \Psi$ and $\Lambda \theta_+$ terms.  

For the $\Lambda \theta_+$ term, by Cordoba's inequality (c.f. \cite{CoIg.fraclap}),
\begin{equation*} \begin{aligned}
\int \varphi \theta_+ \Lambda \theta_+ &\geq \frac{1}{2} \int \varphi \Lambda \theta_+^2
\\ &= \int \theta_+^2 \Lambda \varphi.
\end{aligned} \end{equation*}
...
\end{aligned} \]
For $T$, $\mu$, $\lambda$, and $\varphi$ fixed, this term vanishes in the limit as $\eps \to 0$.  Moreover, the remaining terms in \eqref{local condition with eps term} are all continuous under $L^2$ limits.  It follows that $\theta$ satisfies \eqref{local condition}.  
