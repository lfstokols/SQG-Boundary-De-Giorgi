...
\begin{definition}
A pair $\theta, u \in L^\infty(0,T;L^2(\Omega))$ is called \textbf{suitable} if, for any smooth non-negative function $\Psi \in C^\infty([0,\infty)\times \R^2)$, there exists a constant $C$ depending on $\norm{\grad\Psi}_{L^\infty([0,\infty)\times\R^2)}$ and the H\"{o}lder seminorm $\sup_{[0,\infty)} \bracket{\Psi(t,\cdot)}_{1/4; \R^2}$ such that the following holds:

For any $\lambda >0$ and $\eps \in (0,1)$, and any smooth non-negative $\varphi \in C^\infty_0(0,T;C^\infty(\tilde{\Omega}))$ defined on $\tilde{\Omega} := \{x \in \R^2: \eps x \in \Omega\}$, the functions $\tilde{\theta}_+(t,x) := (\lambda \theta(\eps t, \eps x)-\Psi(t, x))_+$ and $\tilde{u}(t,x):= u(\eps t, \eps x)$ satisfy the inequalities
\begin{equation}\label{leray-hopf condition}
\ddt \int \tilde{\theta}_+^2 + \int \abs{\Lambda^{1/2} \tilde{\theta}_+}^2 \leq C \paren{ \int \indic{\tilde{\theta}_+ \geq 0} + \abs{\int \tilde{\theta}_+ (\del_t \Psi + \tilde{u}\cdot\grad\Psi)} }
\end{equation}
for all $t \in [0,T]$, and
\begin{equation} \label{local condition} \begin{aligned}
\frac{1}{2} \int_0^T\int \varphi \del_t \tilde{\theta}_+^2 &\leq \frac{1}{2} \int_0^T\int \tilde{\theta}_+^2 \tilde{u}\cdot\grad\varphi
+ C\abs{\int_0^T\int \varphi \tilde{\theta}_+ (\del_t\Psi + \tilde{u}\cdot\grad\Psi)}
\\ & + C\norm{\varphi}_{C^0(0,T;C^1(\tilde{\Omega}))} \paren{\abs{\int_0^T\int \tilde{\theta}_+ (\del_t\Psi + \tilde{u}\cdot\grad\Psi)} 
+ \sup_{[0,T]} \int \tilde{\theta}_+^2 + \int_0^T |\{\tilde{\theta}_+ \geq 0\}|^2 + 1}.
\end{aligned}\end{equation}
\end{definition}
Suitability is considered because it is stable under weak limits.  We will show that this property holds for a fairly large class of weak solutions to \eqref{eq:main nonlinear}.

The main result of this paper is the following:
\begin{theorem} \label{thm:main continuity}
Let $\theta$ and $u$ be a suitable weak solution to \eqref{eq:main nonlinear} with initial data $\theta_0 \in L^2(\Omega)$ on a bounded open set $\Omega \subseteq \R^2$ with $C^{2,\beta}$ boundary, $\beta \in (0,1)$, and on a time interval $[0,T]$.  

\vskip.3cm

Then for any $t \in (0,T)$, $\theta$ is bounded and H\"{o}lder continuous in time and space uniformly on $(t,T)\times\bar{\Omega}$.  
\vskip0.3cm
...
Moreover, any weak solution $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; H_0^1(\Omega))$ and corresponding $u$ will be a suitable pair.  
...
Our result applies to weak solutions $\theta \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; H_0^1(\Omega))$.  In \cite{CoNg.strong}, the authors demonstrate local-in-time existence for solutions to \eqref{eq:main nonlinear} with this regularity, assuming initial data in $H^2(\Omega) \cap H_0^1(\Omega)$.  Note that our $C^\alpha$ bound on the solutions depends only on the $L^2$ norm of the initial data.  
\vskip.3cm
...
In order to prove this proposition, we require an energy inequality for $(\theta-C)_+$ for arbitrary constants $C$, which follows from the suitability condition.  We will first prove that all sufficiently regular solutions to $\eqref{eq:main linear}$ are suitable.  
...
\begin{equation} \label{first form of cacciopolli} \paren{\frac{1}{2}} \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2}\theta_+ \Lambda^{1/2} \theta_- + \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \Psi \leq -\int \theta_+ (\del_t \Psi + u \cdot \grad \Psi). \end{equation}
...
\begin{equation}\label{first energy inequality} \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 - \int \Lambda^{1/2} \theta_+ \Lambda^{1/2} \theta_- \leq C \paren{ - \int \theta_+ (\del_t\Psi+u\cdot\grad\Psi) + \int \indic{\theta_+>0} }.\end{equation}

\vskip.3cm%-----%-----%-----%-----%-----%-----%-----%-----%-----%-----%-----%-----

We begin now the proof of \eqref{second energy inequality}.  Since $\theta_+ \in L^\infty(0,T; L^2(\Omega)) \cap L^2(0,T; H_0^1(\Omega))$, by interpolation we can further conclude $\theta_+,u \in L^4(0,T; L^4(\Omega)$.  Therefore we can multiply \eqref{eq:main linear} by $\varphi \theta_+$ and integrate in space to obtain
...
\[ - \int \varphi \theta_+ \Lambda \Psi \leq C\paren{\norm{\grad\Psi}_\infty, [\Psi]_\gamma} \int \indic{\theta_+>0} \,dx \norm{\varphi \theta_+}_{\HD^{1/2}}. \]
...
\\ & \hspace{2cm}+ C \norm{\varphi}_{C^{0,1}(\Omega)} \paren{\norm{\theta_+}_2 + \norm{\theta_+}_{\HD^{1/2}}} \paren{\norm{\theta_+}_{\HD^{1/2}} + \int \indic{\theta_+>0} \,dx} \end{aligned}\]
where $C$ is a constant depending on $\norm{\grad\Psi}_\infty$ and $[\Psi]_\gamma$.

From the bound \eqref{first energy inequality}, we know that 
\[ \norm{\theta_+}_{L^2(0,T;\HD^{1/2})}^2 \leq C \paren{\norm{\theta_+}_{L^\infty(0,T;L^2(\Omega))}^2 + \iint \indic{\theta_+ \geq 0} + \abs{\iint \theta_+ \paren{\del_t\Psi + u\cdot\grad\Psi} }}. \]

We conclude that
\[\begin{aligned} \frac{1}{2} \iint \theta_+^2 \varphi &\leq \frac{1}{2} \iint \theta_+^2 u\cdot\grad\varphi + C \abs{\iint \varphi \theta_+ \paren{\del_t\Psi+u\cdot\grad\Psi}}
\\ & + C\norm{\varphi}_{C^0(0,T;C^{0,1})} \paren{\abs{\iint\! \theta_+ \paren{\del_t\Psi + u\cdot\grad\Psi}} 
+ \sup_{[0,T]} \int\! \theta_+^2 + \int_0^T\! |\{\theta_+\geq 0\}| + \int_0^T\! |\{\theta_+ \geq 0\}|^2}. \end{aligned}\]
...
In order to prove the De Giorgi lemmas, we must derive an energy inequality for the function $\paren{\theta - \Psi}_+$ where $\Psi(t,x)$ grows sublinearly in $|x|$.  However, applying Lemma~\ref{thm:caccioppoli} to such a function, we see that control can only be gained if the quantity $\del_t \Psi + u \cdot \grad\Psi$ is bounded.  This requires a barrier function which is moving in space along a Lagrangian path of $\ulow$.  That Lagrangian path must further be broken up into two parts, one corresponding to the lowest frequencies and one corresponding to mid-range frequencies.  
\vskip.3cm
...
Generally speaking $\ulow$ and $\gamma$ will be locally Lipschitz functions while $\uhigh$ is merely in a weak space $W^{-1/4,\infty}$ and $\Gamma$ will trace out points in $\Omega$ where $\theta$ is well behaved by assumption.  See Section~\ref{sec:holder} for the construction of these functions $\Gamma$ and $\gamma$.  
\vskip.3cm
