...
\[ u = \ulow + \uhigh \]
where $\Lambda^{-1/4} \uhigh \in L^\infty(0,T; L^\infty(\Omega))$ and $\grad \ulow \in L^\infty(0,T; L^\infty(\Omega))$, and assume that $\ulow(\cdot,0) \in L^\infty(0,T)$.  
Then there exists a $\psi \in L^\infty(0,T; L^2(\Omega))$ such that
\[ \theta = \theta_+ + \psi - \theta_- \]
satisfies the inequality
\[ \ddt \int \theta_+^2 + \int \abs{\Lambda^{1/2} \theta_+}^2 \leq C \paren{ \int \indic{\theta_+ > 0} + \int \theta_+ (\del_t \psi + u\cdot\grad\psi) } \]
with the constant $C$ depending on $\norm{\grad \psi}_\infty$ and $\sup_t \bracket{\psi(t,\cdot)}_\gamma$.  
...
\begin{proof}
Define $\gamma(t)$ for $t \in [0,T]$ such that $\gamma(T)=0$ and $\dot{\gamma}(t) = \ulow(t,0)$.  Note that $\gamma$ is Lipschitz and $L^\infty$.  
...
We'll break up $u$ into its constituent parts, and begin by considering the high-pass term.  
\[ \int \Lambda^{-1/4} \uhigh \cdot \Lambda^{1/4} (\theta_+ \grad \phi) = \iint [\Lambda^{-1/4} \uhigh(x)-\Lambda^{-1/4} \uhigh(y)][\theta_+(x) \grad\phi(x+\gamma) - \theta_+(y)\grad\phi(y+\gamma)] K_{1/4}(x,y) + \int \Lambda^{-1/4} \uhigh \theta_+ \grad\phi B. \]
...
\end{align*} 
This bound is sufficient because for the $L^2$ theory, it's clear that you can interpolate between $H^s$ for different values of $s$.  Just think about the sum of eigenvectors, the $H^s$ norm is literally defined as a weighted $\ell^2$ norm, and weights interpolate just through H\"{o}lder's inequality. 
...
Now we have the energy inequality, let's try to do some DG1 and DG2.  

\begin{lemma}
Let $\theta$ be a function in the correct spaces, and let $u = \uhigh + \ulow$ have the proper bounds.  

Let there be a path $\Gamma:[0,T] \to \R^2$ and assume that 
\[ \int_0^T \int_{B_2(\Gamma)} \theta_+^2 \leq \delta_0. \]

Assume also that 
\[ \sup_{[0,T]} \norm{\ulow(t,\cdot) - \dot{\Gamma}(t)}_\infty \leq C. \]

I should work out the scaling before I write this lemma.
\end{lemma}
...
We have a function $\theta$ which is in $L^\infty$, say less than 1, and solves the desired PDE with a drift velocity that decomposes as desired.  Moreover, $\ulow(t,\cdot)$ is a finite sum of $L^\infty$ functions and hence it's $L^\infty$ norm is bounded by a constant $C_0$ that depends only on $\Omega$.  

First we construct a cutoff $\phi$.  Since $\theta \in L^\infty$, we know that $(\theta-\phi)_+$ is supported in a ball of radius 2 around the center $\gamma_0(t)$ of the function $\phi$.  In particular, if we scale down enough times then $\theta_+$ being bounded will lower bound in turn the amount of mass we lose off $\theta_+$ as we scale and eventually we'll get an $L^\infty$ bound of $1-\lambda$ in the ball of radius 1 around $\gamma_0(t)$.  In particular in particular, we get an $L^\infty$ bound on the ball of radius $\eps$ around $0$ for $t \in [0,\frac{1-\eps}{C_0}]$.  

Now we scale down in time and space by $\eps$.  We retain a bound of 1 in the ball of radius $1/\eps$ around $1/\eps \gamma_0(\eps t)$.  It's wholly unclear to me how we keep $\gamma_0$ near $\gamma_1$ since the functions are so nonlinear.  
