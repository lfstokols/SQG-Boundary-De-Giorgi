...
Suppose that for $t \in [-5,0]$, $\theta(t,x)$ has some tbd growth condition on $|x-\Gamma| > 3$ and $\theta \leq 2$ when $|x-\Gamma| \leq 3$.  
...
\[ \theta(t,x) \leq 1 + 2^{-k_0} \paren{|x-\Gamma(t)|^{1/4}-1}_+ \]
and
\[ \abs{\{\theta \leq 0\} \cap [-4,0]\times B_4(\Gamma)} \geq \frac{4|B_4|}{2} \]
then for all $t \in [-1,0]$, $x \in B_{1/2}(\Gamma)$ we have
\[ \theta(t,x) \leq 1-\lambda. \]
\end{proposition}

\begin{proof}
Let $\mu$ and $\delta_0$ as in [citation] the lemmas DG1 and DG2, and take $k_0$ large enough that $k_0 \mu > 4 |B_4|$.  

Consider the sequence of functions
\[ \theta_k(t,x) := 1 + 2^k (\theta(t,x) - 1). \]
That is, $\theta_0 = \theta$ and as $k$ increases, we scale vertically by a factor of 2 while keeping 1 fixed.  Note that since $\theta$ satisfies [cite, boundedness], each $\theta_k$ for $k \leq k_0$ and $(t,x) \in [-5,0] \times \Omega$ satisfies
\[ \theta_k(t,x) \leq 1 + \paren{|x-\Gamma(t)|^{1/4}-1}_+. \]
...
for $k = k_0$.  This is nonsense, that quantity is trivially 0 by assumption.  
