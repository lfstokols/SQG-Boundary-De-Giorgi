...
Moreover, for any time $t > 0$, there exists a constant $C$ depending only on $t$ and $\Omega$ such that
\[ \norm{\theta}_{L^\infty([t,\infty)\times\bar{\Omega})} \leq C \norm{\theta_0}_{L^2(\Omega)} \]
...
Then for any smooth non-negative function $\Psi \in C^\infty([0,\infty)\times \R^2)$ satisfying $\norm{\grad\Psi}_{L^\infty([0,\infty)\times\R^2)} \leq k$ and the H\"{o}lder seminorm $\sup_{[0,\infty)} \bracket{\Psi(t,\cdot)}_{1/4; \R^2} \leq k$ for some constant $k$ and any smooth non-negative $\varphi \in \Ctest(S,T;\Ctest(\Omega))$, 
...
\frac{-1}{2} \int_S^T\!\!\!\!\int\! \theta_+^2 \del_t \varphi \leq -\int_S^T\!\!\!\!\int\! \varphi \theta_+ \paren{\del_t \Psi + u\cdot\grad\Psi} - \frac{1}{2} \int_S^T\!\!\!\!\int\! \theta_+^2 u\cdot\grad\varphi - \frac{1}{2} \int_S^T\!\!\!\!\int\! \theta_+^2 \Lambda \varphi - \int_S^T\!\!\!\!\int\! \varphi \theta_+ \Lambda \Psi.
%\begin{aligned}
...
Define $\tilde{\Omega} := \{x \in \R^2: \mu x \in \Omega\}$, $\tilde{\theta}_+(t,x) := (\lambda \theta(\mu t, \mu x)-\Psi(t, x))_+$, and $\tilde{u}(t,x):= u(\mu t, \mu x)$.  Let $\varphi \in \Ctest(S,T;\Ctest(\tilde{\Omega}))$ be non-negative.  
...
By the standard De Giorgi argument (see Lemma~\ref{thm:DG1 skeleton} in the Appendix for details), this guarantees that $\theta \in L^\infty([t,\infty]\times\Omega)$ for all $t > 0$, with norm depending on $t$ and $\norm{\theta_0}_{L^2(\Omega)}$ and $\Omega$ but independent of $\eps$.  
\end{proof}
...
\begin{proof}
For any parameter $\eps > 0$, define $\theta_\eps \in L^2(H_0^1)$ the weak solution to \eqref{eq:viscous} constructed in Lemma~\ref{thm:viscous}.  The $\theta_\eps$ are uniformly bounded in $L^\infty(L^2)$ and $L^2(\HD^{1/2})$, so by interpolation they are also uniformly bounded in $L^4(L^{8/3})$.  
...
Let $0 < T < \infty$ be a constant time, and let $\Psi$, $\varphi$, $\lambda$, and $\mu$ be as in the definition of suitable solutions.  Define $\tilde{\Omega} := \{x \in \R^2: \mu x \in \Omega\}$, $\tilde{T} := \mu\n T$, $\tilde{\theta}_+(t,x) := \paren{\lambda \theta_\eps(\mu t, \mu x) - \Psi(t,x)}_+$, and $\tilde{u}(t,x) := u(\mu t, \mu x)$.  
...
%\begin{equation} \label{local condition with eps term} \begin{aligned} 
%\frac{-1}{2} \iint\!\! \thetaep^2 \del_t \varphi
%&\leq \frac{1}{2} \iint\!\! \thetaep^2 \ue\cdot\grad\varphi 
%+ \abs{\iint\!\! \varphi \thetaep \paren{\del_t\Psi+\ue\cdot\grad\Psi}} 
%+ C\norm{\varphi}_{C^0(0,{\tilde{T}};C^{0,1})} \Bigg(\iint\!\! \thetaep^2 
%\\ & + \sup_{[0,\tilde{T}]} \int\!\! \thetaep^2 
%+ k^2 \int_0^{\tilde{T}}\!\! |\{\thetaep\geq 0\}| 
%+ \abs{\iint\!\! \thetaep \paren{\del_t\Psi + \ue\cdot\grad\Psi}} 
%+ k^4 \int_0^{\tilde{T}}\!\! |\{\thetaep \geq 0\}|^2 
%\\ & + \eps \frac{k^2}{2} \mu\n T \mu^{-2} |\Omega| \Bigg) 
%+ \eps \iint\!\! \varphi \thetaep \Laplace \thetae. 
%\end{aligned} \end{equation}
...
$L^2 \to L^\infty$ regularization is a straightforward application of the standard De Giorgi iteration argument.  Since we make this argument multiple times in this paper, the details are sketched in the Appendix, see Lemma~\ref{thm:DG1 skeleton}.  
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\Gammal:[-T,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} hold on $\Omega$ with kernels that satisfy
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$, and let $\Gammal:[-2,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} holds on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{1/2} K_{1}(x,y). \]
...
Let $\Omega \subseteq \R^2$ be a bounded open set with $C^{2,\beta}$ boundary for some $\beta \in (0,1)$ , and let $\Gammal:[-5,0] \to \R^2$ be Lipschitz.  Assume that Proposition~\ref{thm:Caff Stinga representation} holds on $\Omega$ with kernels that satisfy
\[ K_{1/4}(x,y) \leq \Comega |x-y|^{1/2} K_{1}(x,y). \]
