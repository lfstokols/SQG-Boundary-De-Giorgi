...
Then there exists a constant $\delta_0$ such that
\[ \int_{-2}^0 \int_{\Gamma(t)+B_2} \max(\theta,0)^2 \,dxdt \leq \delta_0 \]
implies that
\[ \theta(t,x) \leq 1 \qquad \forall (t,x) \in [-1,0]\times B_\eps(\Gamma(t)). \]

\end{lemma}

\begin{proof}
Let $\phi$ be such that $\phi = 0$ on $B_1$ and $\phi(x) \leq |x|^{1/4}$ for $|x|>2$ while $\phi$ is Lipschitz and H\"{o}lder continuous with exponent $1/4$.  

Consider the sequence of functions
\[ \theta_k := (\theta(t,x) - \phi(x - \Gamma(t) - \gamma(t)) - 1 + 2^{-k})_+ \]
and define
\[ \E_k := \int_{-1-2^{-k}}^0 \int_\Omega \theta_k^2 \,dxdt. \]

Notice that by assumption
\[ \E_0 = \int_{-2}^0 \int_\Omega (\theta - \phi(x-\Gamma-\gamma))_+ \,dxdt \leq \]
