...
\begin{equation}\label{DG2 energy} \sup_{[-4,0]} \int \theta_+^2 + \int_{-4}^0 \int \abs{\Lambda^{1/2}\theta_+}^2 + \int_{-4}^0 \int \Lambda^{1/2}\theta_+ \Lambda^{1/2}\theta_- \leq C \int_0^5 \int \paren{\indic{\theta_+>0} + \theta_+ + \theta_+^2}. \end{equation}
...
\[ \iint \varphi(x) [\theta_+(x)^2 - \theta_+(y)^2][\psi(x)-\psi(y)] K + \int \theta_+(y)^2\int[\varphi(x) - \varphi(y)][\psi(x)-\psi(y)] K. \]
The first part is bounded (using H\"{o}lder's inequality) by the $L^2$ norm of $\varphi$ since $\psi$ is Lipschitz and $\theta_+^2 \in \HD^{1/2}$, while the second part is bounded by the $C^1$ norms of $\varphi$ and $\psi$ and the $L^2$ norm of $\theta_+$.  
